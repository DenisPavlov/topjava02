...
public class DataJpaMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        Optional<User> optUser = crudUserRepository.findById(userId);
        if (optUser.isEmpty()) {
            return null;
        } else {
            meal.setUser(optUser.get());
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return crudMealRepository.findAllByUserIdAndDateTimeBetween(userId, startDate, endDate, SORT_DATETIME);
    }
}
